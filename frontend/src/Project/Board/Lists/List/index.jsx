...
const filterIssues = (projectIssues, filters, currentUserId) => {
  ...
  let user_id = issues.user_id ? issues.user_id.split(',') : [];

  if (searchTerm) {
    issues = issues.filter(issue => issue.title.toLowerCase().includes(searchTerm.toLowerCase()));
  }
  if (userIds.length > 0) {
    issues = issues.filter(issue => intersection(user_id, userIds).length > 0);
  }
  if (myOnly && currentUserId) {
    issues = issues.filter(issue => user_id.includes(currentUserId));
  }
  if (recent) {
...
const getSortedListIssues = (issues, status) =>
  issues.filter(issue => issue.state === status).sort((a, b) => a.listPosition - b.listPosition);
