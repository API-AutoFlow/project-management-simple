...
const ProjectIssueSearch = ({ project }) => {
  ...
  const [{ data, isLoading }, fetchIssues] = useApi.get('/board/search', {}, { lazy: true });

  const matchingIssues = get(data, 'result', []) ? get(data, 'result', []) : [];

  const recentIssues = sortByNewest(project.issues, 'created_at').slice(0, 10);
  ...
  const handleSearchChange = value => {
    ...
    if (searchTerm) {
      fetchIssues({ project_id: project.id, value: searchTerm });
    }
  };
