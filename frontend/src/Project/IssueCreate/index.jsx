...
const ProjectIssueCreate = ({ project, fetchProject, onCreate, modalClose }) => {
  const [{ isCreating }, createIssue] = useApi.post('/board');
      ...
      onSubmit={async (values, form) => {
        try {
          await createIssue({
            ...
            users: values.assignees.map(id => ({ id })),
          });
          await fetchProject();
...
const userOptions = project =>
  project.users.map(user => ({ value: user.id, label: `${user.first_name} ${user.last_name}` }));
...
const renderUser = project => ({ value: userId, removeOptionValue }) => {
  ...
  return (
    ...
    >
      <Avatar
        size={20}
        avatarUrl={user.progile_image}
        name={`${user.first_name} ${user.last_name}`}
      />
      <SelectItemLabel>{`${user.first_name} ${user.last_name}`}</SelectItemLabel>
